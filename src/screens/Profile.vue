            ...
            <form @submit.prevent="changePassword">
              <div class="mb-4 flex flex-col md:flex-row gap-4">
                ...
                <div class="w-full md:w-1/3">
                  ...
                  <div class="relative">
                    <input id="confirmNewPassword" v-model="passwordForm.confirmNewPassword" type="password"
                      class="w-full border rounded px-3 py-2 pl-10 transition-all duration-300 ease-in-out hover:rounded-2xl"
                      required />
                    <PasswordIcon />
                  </div>
              ...
              <button type="submit" @click.prevent="changePasswordUtil"
                class="bg-primary text-white px-4 py-2 rounded hover:bg-primary-dark flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                  stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
...
const changePasswordUtil = async () => {
  errorMessage.value = "";
